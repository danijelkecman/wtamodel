...
struct ThreatEditorView: View {
    ...
    /// With a global budget `G`, this is `G − Σ(other threats’ caps)` so this threat’s cap keeps Σ caps ≤ `G`. Use `Int.max` when there is no global budget.
    let residualPerThreatCapBudget: Int
    let onRemove: () -> Void
    /// Called when value, shot model, or name changes — usually triggers a solver refresh.
    let onRecalculate: () -> Void

    /// Max selectable cap for this threat: never above **Interceptor count**, and never above what’s left after other threats’ caps (so Σ caps ≤ Interceptor count).
